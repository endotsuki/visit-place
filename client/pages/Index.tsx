...
export default function Index() {
  ...
  const [phnomPenhDistances, setPhnomPenhDistances] = useState<
    Record<string, number>
  >({});
  const [provinces, setProvinces] = useState<Set<string>>(new Set());

  // Phnom Penh coordinates as fallback
  const PHNOM_PENH = { lat: 11.5749, lng: 104.9139 };
    ...
    const calculateDistances = async () => {
      ...
      for (const place of places) {
        ...
        try {
          ...
          console.error(
            `Error calculating distance for place ${place.id}:`,
            error,
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (searchKeywords.trim()) {
      ...
      filtered = filtered.filter((place) =>
        place.keywords.some((kw) => kw.toLowerCase().includes(keywordsLower)),
      );
    }
          ...
          {loading ? (
            <div className="flex justify-center items-center py-12">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">
                  {isKhmer ? "កំពុងផ្ទុក..." : "Loading places..."}
                </p>
              </div>
            </div>
            ...
            <>
              <p className="text-muted-foreground mb-6">
                {isKhmer
                  ? `បានរកឃើញ ${filteredPlaces.length} កន្លែង`
